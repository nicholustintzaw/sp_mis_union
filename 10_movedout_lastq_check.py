...
for file in offices :
    ...
    for xlsx in xlsxs :
        ...
        if xlsx.endswith(".xlsx"):
            ...
            dta_raw = pd.read_excel(raw + file + '/_raw/' + xlsx, sheet_name = '05_moved_out', \
...
if obs > 0 :
    ...
    merge_j = pd.merge(df_last_x, df_moveout, right_on = ['benef_id'] , 
    ...
    writer = pd.ExcelWriter(output + '05_moveout_notfound_olddata.xlsx', engine = 'xlsxwriter')
    ...
    merge_j = pd.merge(df_moveout, df_movein, on = ['benef_id', 'Benef: Name'] , suffixes = ('_last', '_new'), 
                 indicator = 'merge', how = 'outer')
    ...
    if obs_x > 0 | obs_y > 0 :    
        writer = pd.ExcelWriter(output + '02_05_moved_inout_check.xlsx', engine = 'xlsxwriter')
