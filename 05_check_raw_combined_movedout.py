...
purpose         :           quaterly data combine and check - moved out
...
print('Now, the spyder is working on Moved out Sheet, please wait for a few minutes')
...
col_names = ['No.', 'benef_id', 'Benef: Name', 'Status: Date',
       'Status Month', 'Status: Year', 'Status Date',
       'Moved: State/Region Name', 'Moved: cal_region',
       'Moved: State/Region Code', 'Moved: District Name', 'cal_district',
       'Moved: Township Name', 'cal_move_dist_ts', 'cal_town',
       'Moved: Township Code', 'Rural or Urban', 'Moved: Ward',
...
col_comb = ['No.', 'benef_id', 'Benef: Name', 'Status: Date',
       'Status Month', 'Status: Year', 'Status Date',
       'Moved: State/Region Name', 'Moved: cal_region',
       'Moved: State/Region Code', 'Moved: District Name', 'cal_district',
       'Moved: Township Name', 'cal_move_dist_ts', 'cal_town',
...
for file in offices :
    ...
    for xlsx in xlsxs :
        ...
        if xlsx.endswith(".xlsx"):
            ...
            dta = pd.read_excel(raw + file + '/_raw/' + xlsx, sheet_name = '05_moved_out', \
                            skiprows = 3, header = None, index_col = False, usecols="A:U", names = col_names)
    ...
    for xlsx in xlsxs :
        ...
        if xlsx.endswith(".xlsx"):
            ...
            df_comb = pd.read_excel(raw + file + '/_combined/' + xlsx, sheet_name = '05_moved_out', 
                                     skiprows = 1, header = None, index_col = False, usecols="B:W", names = col_comb)
    ...
    merge_j = {'State and Region': [file], 'Raw Moved Out': [tot_raw], 'Raw ID Duplicate': [dupid_raw], 'Raw Person Duplicate': [dupresp_raw],
               'Combined Moved Out': [tot_comb], 'Combined ID Duplicate': [dupid_comb], 'Combined Person Duplicate': [dupresp_comb],
    ...
    sum_merge.to_excel(output + '05_movedout_raw_combined_check_result.xlsx', index = False)
    
   
    writer = pd.ExcelWriter(output + file + '/' + file + '_duplicated_movedout_results.xlsx', engine = 'xlsxwriter')    
    obs = len(dup_id_raw)
    if obs > 0 : 
    ...
    if obs_z > 0 | obs_y >0 :  
        writer = pd.ExcelWriter(output + file + '/' + file + '_unmatched_movedout_results.xlsx', engine = 'xlsxwriter')    
        obs = len(z_j)
        if obs > 0 : 
...
print('Woow, just finished the Moved out Sheet checking, please check your outputs folder for result excel files')
