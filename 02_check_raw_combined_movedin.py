...
col_match = ['benef_id', 'Benef: Name', 'Benef: Gender', 'Benef: Father Name']

col_comb = ['No.', 'State/Region Name', 'cal_region', 'State/Region Code',
       'District Name', 'cal_district', 'Township Name', 'cal_dist_town',
       'cal_town', 'Township Code', 'Rural or Urban', 'Ward', 'Village Tract',
       'Village', 'Address Detail', 'benef_id', 'Benef: Name', 'Benef: Gender',
       'Benef: Father Name', 'source']
...
for file in offices :
    ...
    for xlsx in xlsxs :
        ...
        if xlsx.endswith(".xlsx"):
            ...
            dta = pd.read_excel(raw + file + '/_raw/' + xlsx, sheet_name = '02_moved_in', 
    ...
    for xlsx in xlsxs :
        ...
        if xlsx.endswith(".xlsx"):
            ...
            df_comb = pd.read_excel(raw + file + '/_combined/' + xlsx, sheet_name = '02_moved_in', 
                                     skiprows = 1, header = None, index_col = False, usecols="B:U", names = col_comb)
    ...
    #obs_raw  = len(df_raw)
    #obs_comb = len(df_comb)
    
    #if obs_raw != 0 & obs_comb != 0 :
    ...
    merge_j = {'State and Region': [file], 'Raw Moved In': [tot_raw], 'Raw ID Duplicate': [dupid_raw], 'Raw Person Duplicate': [dupresp_raw],
               'Combined Moved In': [tot_comb], 'Combined ID Duplicate': [dupid_comb], 'Combined Person Duplicate': [dupresp_comb],
    ...
    sum_merge.to_excel(output + '02_movedin_raw_combined_check_result.xlsx', index = False)
    
    writer = pd.ExcelWriter(output + file + '/' + file + '_duplicated_movedin_results.xlsx', engine = 'xlsxwriter')    
    obs = len(dup_id_raw)
    if obs > 0 : 
    ...
    if obs_z > 0 | obs_y >0 :         
        writer = pd.ExcelWriter(output + file + '/' + file + '_unmatched_movedin_results.xlsx', engine = 'xlsxwriter')    
        obs = len(z_j)
        if obs > 0 : 
