...
for file in offices :
    ...
    for xlsx in xlsxs :
        ...
        if xlsx.endswith(".xlsx"):
            ...
            dta = pd.read_excel(raw + file + '/_raw/' + xlsx, sheet_name = '06_false_register', \
    ...
    for xlsx in xlsxs :
        ...
        if xlsx.endswith(".xlsx"):
            ...
            df_comb = pd.read_excel(raw + file + '/_combined/' + xlsx, sheet_name = '06_false_register', 
    ...
    merge_j = {'State and Region': [file], 'Raw False Register': [tot_raw], 'Raw ID Duplicate': [dupid_raw], 'Raw Person Duplicate': [dupresp_raw],
               'Combined False Register': [tot_comb], 'Combined ID Duplicate': [dupid_comb], 'Combined Person Duplicate': [dupresp_comb],
    ...
    sum_merge.to_excel(output + '06_false_register_raw_combined_check_result.xlsx', index = False)
    
   
    writer = pd.ExcelWriter(output + file + '/' + file + '_duplicated_falseregister_results.xlsx', engine = 'xlsxwriter')    
    obs = len(dup_id_raw)
    if obs > 0 : 
    ...
    if obs_z > 0 | obs_y >0 :  
        writer = pd.ExcelWriter(output + file + '/' + file + '_unmatched_falseregister_results.xlsx', engine = 'xlsxwriter')    
        obs = len(z_j)
        if obs > 0 : 
