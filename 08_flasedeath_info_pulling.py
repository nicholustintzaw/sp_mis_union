...
for file in offices :
    ...
    for xlsx in xlsxs :
        ...
        if xlsx.endswith(".xlsx"):
            ...
            dta_raw = pd.read_excel(raw + file + '/_raw/' + xlsx, sheet_name = '03_false_death_in', \
...
if obs > 0 :
    ...
    merge_j = pd.merge(df_last_x, df_falsed, right_on = ['benef_id'] , 
    ...
    if obs_x > 0 | obs_y > 0 :  
        writer = pd.ExcelWriter(output + '03_falsedeath_notfound_olddata.xlsx', engine = 'xlsxwriter')
...
print('Woow, just finished the false death in Sheet checking, please check your outputs folder for result excel files')
